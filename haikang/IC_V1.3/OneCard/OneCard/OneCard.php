...
class OneCard extends ApiBase
{
    ...
    public function simpleDownloadTask($in_data){
        ...
        if(!isset($in_data['personInfo']['startTime'])){
            $this->error[] = '开始时间必须';
        }else{
            $array['personInfo']['startTime'] = date('Y-m-d\TH:i:s+08:00',$in_data['personInfo']['startTime']);
        }
        if(!isset($in_data['personInfo']['endTime'])){
            $this->error[] = '结束时间必须';
        }else{
            $array['personInfo']['endTime'] = date('Y-m-d\TH:i:s+08:00',$in_data['personInfo']['endTime']);
        }
        $this->request_data = $array;
        return true;
    }
